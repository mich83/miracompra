$ ->
  ...
  Spree.showVariantImages = (variantId) ->
    ...
    if not currentThumb.hasClass('vtmb-' + variantId)
      ...
      ($ '#main-image a.large-image').attr 'rel', 'gallery-'+variantId
  ...
  Spree.updateVariantPrice = (variant) ->
    ...
    variantPriceWithDiscount = variant.data('price-with-discount')
    variantSku = variant.data('sku')
    ($ '.price-with-discount.selling').text(variantPriceWithDiscount) if variantPriceWithDiscount
    ($ '.price.selling').text(variantPrice) if variantPrice
    ($ '#product-code').text(variantSku) if variantSku
